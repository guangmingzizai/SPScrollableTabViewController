...
- (void)__setupHairline
{
    if ([self.tabDataSource respondsToSelector:@selector(topHairlineColor)]) {
        UIColor *color = [self.tabDataSource topHairlineColor];
        UIEdgeInsets insets = UIEdgeInsetsZero;
        if ([self.tabDataSource respondsToSelector:@selector(topHairlineInsets)]) {
            insets = [self.tabDataSource topHairlineInsets];
        }
        UIView *hairline = [[UIView alloc] initWithFrame:CGRectMake(insets.left, insets.top, self.frame.size.width - (insets.left + insets.right), 2)];
        hairline.backgroundColor = color;
        [self addSubview: hairline];
    }
    
    if ([self.tabDataSource respondsToSelector:@selector(bottomHairlineColor)]) {
        UIColor *color = [self.tabDataSource bottomHairlineColor];
        UIEdgeInsets insets = UIEdgeInsetsZero;
        if ([self.tabDataSource respondsToSelector:@selector(bottomHairlineInsets)]) {
            insets = [self.tabDataSource bottomHairlineInsets];
        }
        UIView *hairline = [[UIView alloc] initWithFrame:CGRectMake(insets.left, self.frame.size.height - insets.bottom - 2, self.frame.size.width - (insets.left + insets.right), 2)];
        hairline.backgroundColor = color;
        [self addSubview: hairline];
    }
}
...
- (void)__setupTagBarView
{
    ...
    for (int i=0;i<[self.tabDataSource numberOfTab];i++) {
        ...
        [self reloadTabBarItemAppearanceAtIndex:i itemView:titleView];
...
-(void)reloadTabBarTitleAppearance
{
    for (int i=0;i<[self.tabDataSource numberOfTab];i++) {
...
- (void)reloadTabBarItemAppearanceAtIndex:(NSInteger)index itemView:(SPPageTagTitleView *)itemView {
    if ([self.tabDataSource respondsToSelector:@selector(titleColorForIndex:)]) {
        itemView.normalTitleColor = [self.tabDataSource titleColorForIndex:index];
    } else {
        itemView.normalTitleColor = [UIColor blackColor];
    }
    if ([self.tabDataSource respondsToSelector:@selector(titleHighlightColorForIndex:)]) {
        itemView.highlightedTitleColor = [self.tabDataSource titleHighlightColorForIndex:index];
    } else {
        itemView.highlightedTitleColor = [UIColor orangeColor];
    }
    if ([self.tabDataSource respondsToSelector:@selector(titleFontForIndex:)]) {
        itemView.normalFont = [self.tabDataSource titleFontForIndex:index];
    } else {
        itemView.normalFont = [UIFont systemFontOfSize:15];
    }
    if ([self.tabDataSource respondsToSelector:@selector(titleHighlightFontForIndex:)]) {
        itemView.highlightedFont = [self.tabDataSource titleHighlightFontForIndex:index];
    } else {
        itemView.highlightedFont = [UIFont boldSystemFontOfSize:15];
    }
}
