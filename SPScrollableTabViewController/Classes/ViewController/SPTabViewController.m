...
@interface SPTabViewController () <SPPageViewControllerDelegate, SPPageViewControllerDataSource, SPTabDataSource, SPTabDelegate>

@property (nonatomic, strong) UIView *tabView;

@property (nonatomic, strong) SPPageViewController *pageController;
...
{
    return _pageController.currentPageIndex;
}
...
- (void)__setupPage
{
    self.pageController = [[SPPageViewController alloc] init];
...
#pragma SPPageViewControllerDelegate
...
-(void)scrollViewContentOffsetWithRatio:(CGFloat)ratio draging:(BOOL)draging
{
    if ([self.tabView isKindOfClass:[SPTagBarScrollView class]]) {
        ...
        if (!draging) {
            __weak SPTabViewController *wSelf = self;
            [UIView animateWithDuration:0.3 animations:^{
                SPTabViewController *bSelf = wSelf;
...
- (void)pageviewController:(SPPageViewController*)pageController willTransitionFromVC:(UIViewController *)fromVC toViewController:(UIViewController *)toVC
{
    [self changeToSubControllerOffset:toVC isDelay:NO];
}

- (void)pageviewController:(SPPageViewController*)pageController didTransitionFromVC:(UIViewController *)fromVC toViewController:(UIViewController *)toVC
{
...
- (void)pageviewController:(SPPageViewController*)pageController willLeaveFromVC:(UIViewController *)fromVC toViewController:(UIViewController *)toVC
{
    [self changeToSubControllerOffset:toVC isDelay:NO];
}

- (void)pageviewController:(SPPageViewController*)pageController didLeaveFromVC:(UIViewController *)fromVC toViewController:(UIViewController *)toVC
{
    [self pageviewControllerDidTransitiontoViewController:toVC fromVC:fromVC];
}

#pragma SPPageViewControllerDataSource
...
- (UIFont *)titleHighlightFontForIndex:(NSInteger)index {
...
- (UIEdgeInsets)bottomHairlineInsets {
    return UIEdgeInsetsZero;
}
